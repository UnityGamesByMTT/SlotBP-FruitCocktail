...
public class GameManager : MonoBehaviour
{
    ...
    private void OnClickDetection()
    {
        ...
        if (m_Paytable_Button) m_Paytable_Button.onClick.AddListener(() => { m_PushObject(m_Paytable_Object); });

        if (m_Rules_Button) m_Rules_Button.onClick.RemoveAllListeners();
        if (m_Rules_Button) m_Rules_Button.onClick.AddListener(() => { m_PushObject(m_Rules_Object); });

        if (m_Settings_Button) m_Settings_Button.onClick.RemoveAllListeners();
        if (m_Settings_Button) m_Settings_Button.onClick.AddListener(() => { m_PushObject(m_Settings_Object); });
